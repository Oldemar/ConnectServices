	...
	public function generatepayroll() 
	{
		...
		foreach ($sales as $key => $sale) 
		{
			...
			if ($sale['Sale']['tv'] != 'None') 
			{
				...
				$salesByUsers[$userID]['Total']['Val'][$sale['Sale']['tv']] = 
					($salesByUsers[$userID]['Total']['Qty'][$sale['Sale']['tv']] * 
										$this->Service->getPrice($sale['Sale']['tv'])) *
					$this->Comission->getPerc($sale['User']['region'],$sale['User']['topleader'],$sale['User']['role_id'],$salesByUsers[$userID]['Total']['Qty'][$sale['Sale']['tv']]);
				$salesByUsers[$userID]['Total']['Qty']['TV']++;
				$salesByUsers[$userID]['Total']['Val']['TV'] += $salesByUsers[$userID]['Total']['Val'][$sale['Sale']['tv']];
				$salesByUsers[$userID]['Payroll']['comission'] += $salesByUsers[$userID]['Total']['Val'][$sale['Sale']['tv']];
			}
			if ($sale['Sale']['internet'] != 'None') 
			{
				$salesByUsers[$userID]['Total']['Qty'][$sale['Sale']['internet']]++;
				$salesByUsers[$userID]['Total']['Val'][$sale['Sale']['internet']] = 
					($salesByUsers[$userID]['Total']['Qty'][$sale['Sale']['internet']] * 
										$this->Service->getPrice($sale['Sale']['internet']))*
					$this->Comission->getPerc($sale['User']['region'],$sale['User']['topleader'],$sale['User']['role_id'],$salesByUsers[$userID]['Total']['Qty'][$sale['Sale']['internet']]);
				$salesByUsers[$userID]['Total']['Qty']['INTERNET']++;
				$salesByUsers[$userID]['Total']['Val']['INTERNET'] += $salesByUsers[$userID]['Total']['Val'][$sale['Sale']['internet']];
				$salesByUsers[$userID]['Payroll']['comission'] += $salesByUsers[$userID]['Total']['Val'][$sale['Sale']['internet']];
			}
			if ($sale['Sale']['phone'] != 'None') 
			{
				$salesByUsers[$userID]['Total']['Qty'][$sale['Sale']['phone']]++;
				$salesByUsers[$userID]['Total']['Val'][$sale['Sale']['phone']] = 
					($salesByUsers[$userID]['Total']['Qty'][$sale['Sale']['phone']] * 
										$this->Service->getPrice($sale['Sale']['phone']))*
					$this->Comission->getPerc($sale['User']['region'],$sale['User']['topleader'],$sale['User']['role_id'],$salesByUsers[$userID]['Total']['Qty'][$sale['Sale']['phone']]);
				$salesByUsers[$userID]['Total']['Qty']['PHONE']++;
				$salesByUsers[$userID]['Total']['Val']['PHONE'] += $salesByUsers[$userID]['Total']['Val'][$sale['Sale']['phone']];
				$salesByUsers[$userID]['Payroll']['comission'] += $salesByUsers[$userID]['Total']['Val'][$sale['Sale']['phone']];
			}
			if ($sale['Sale']['homeSecurity'] != 'None') 
			{
				$salesByUsers[$userID]['Total']['Qty'][$sale['Sale']['homeSecurity']]++;
				$salesByUsers[$userID]['Total']['Val'][$sale['Sale']['homeSecurity']] = 
					($salesByUsers[$userID]['Total']['Qty'][$sale['Sale']['homeSecurity']] * 
										$this->Service->getPrice($sale['Sale']['homeSecurity']))*
					$this->Comission->getPerc($sale['User']['region'],$sale['User']['topleader'],$sale['User']['role_id'],$salesByUsers[$userID]['Total']['Qty'][$sale['Sale']['homeSecurity']]);
				$salesByUsers[$userID]['Total']['Qty']['XFINITY HOME']++;
				$salesByUsers[$userID]['Total']['Val']['XFINITY HOME'] += $salesByUsers[$userID]['Total']['Val'][$sale['Sale']['homeSecurity']];
				$salesByUsers[$userID]['Payroll']['comission'] += $salesByUsers[$userID]['Total']['Val'][$sale['Sale']['homeSecurity']];
			}
			if ($sale['Sale']['globo']) 
			{
				$salesByUsers[$userID]['Total']['Qty']['globo']++;
				$salesByUsers[$userID]['Total']['Val']['globo'] = 
					($salesByUsers[$userID]['Total']['Qty']['globo'] * 
										$this->Service->getPrice('globo'))*
					$this->Comission->getPerc($sale['User']['region'],$sale['User']['topleader'],$sale['User']['role_id'],$salesByUsers[$userID]['Total']['Qty']['globo']);
				$salesByUsers[$userID]['Total']['Qty']["EXTRAS"]++;
				$salesByUsers[$userID]['Total']['Val']['EXTRAS'] += $salesByUsers[$userID]['Total']['Val']['globo'];
				$salesByUsers[$userID]['Payroll']['comission'] += $salesByUsers[$userID]['Total']['Val']['globo'];
			}
			if ($sale['Sale']['record']) 
			{
				$salesByUsers[$userID]['Total']['Qty']['record']++;
				$salesByUsers[$userID]['Total']['Val']['record'] = 
					($salesByUsers[$userID]['Total']['Qty']['record'] * 
										$this->Service->getPrice('record'))*
					$this->Comission->getPerc($sale['User']['region'],$sale['User']['topleader'],$sale['User']['role_id'],$salesByUsers[$userID]['Total']['Qty']['record']);
				$salesByUsers[$userID]['Total']['Qty']["EXTRAS"]++;
				$salesByUsers[$userID]['Total']['Val']['EXTRAS'] += $salesByUsers[$userID]['Total']['Val']['record'];
				$salesByUsers[$userID]['Payroll']['comission'] += $salesByUsers[$userID]['Total']['Val']['record'];
			}
			if ($sale['Sale']['pfc']) 
			{
				$salesByUsers[$userID]['Total']['Qty']['pfc']++;
				$salesByUsers[$userID]['Total']['Val']['pfc'] = 
					($salesByUsers[$userID]['Total']['Qty']['pfc'] * 
										$this->Service->getPrice('pfc'))*
					$this->Comission->getPerc($sale['User']['region'],$sale['User']['topleader'],$sale['User']['role_id'],$salesByUsers[$userID]['Total']['Qty']['pfc']);
				$salesByUsers[$userID]['Total']['Qty']["EXTRAS"]++;
				$salesByUsers[$userID]['Total']['Val']['EXTRAS'] += $salesByUsers[$userID]['Total']['Val']['pfc'];
				$salesByUsers[$userID]['Payroll']['comission'] += $salesByUsers[$userID]['Total']['Val']['pfc'];
			}
 ...
 */
	...
	Public  function generate()
	{
		...
		foreach ($this->data as $key => $userInfo) 
		{
			if ($key != '2')
			{
				if ($userInfo['Payroll']['comission'] != 0 && $userInfo['User']['topleader'] == $this->objLoggedUser->getID())
				{
					$this->Payroll->save($userInfo['Payroll']);
					$this->Payroll->id = null;
				}
				if ($userInfo['Saving']['saving'] != 0 && $userInfo['User']['topleader'] == $this->objLoggedUser->getID())
				{
					$this->Saving->save($userInfo['Saving']);
					$this->Saving->id = null;
				}
				if ($userInfo['User']['role_id'] == '4' && 
					$arrTotTvSalesByTopLeader['qtd']+$arrTotNetSalesByTopLeader['qtd']+$arrTotPhSalesByTopLeader['qtd']+$arrTotXhSalesByTopLeader['qtd'] > 0)
				{
				...
				{
					if ($this->User->getTopleader($sale['Sale']['user_id']) == $this->objLoggedUser->getID())
					{
		...
		foreach ($this->data as $key => $userInfo) 
		{
			if ($key != '2')
			{
				$this->Receivable->id = $userInfo['Payroll']['receivable_id'];
				$this->Receivable->savefield('processed','1');
				break;
			}
		}
		exit;
