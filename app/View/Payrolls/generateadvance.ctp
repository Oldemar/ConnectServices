...
echo $line1;
?>	
		<td class="text-right" style="background-color: #ccc"><b>Advance</b></td>
	</tr>
	<tr>
		<td></td>
		<?php echo $line2 ?>
...
foreach ($salesByUsers as $key2 => $sale) 
{
	...
	echo '<td class="text-right" style="background-color: #ccc"><b>'.CakeNumber::currency($sale['Advance']['value'],'USD').'</b></td></tr>';
}
?>
</table>
...
<div class="text-center dispBtn" style="padding-bottom:30px;display:none">
	<?php echo $this->Html->link(__('Back'), array('controller'=>'sales' ,'action'=>'listsales','Advance'), array('class'=>'btn btn-lg btn-primary')); ?></li>
</div>
<script type="text/javascript">
	$('#genPayRoll').click(function(e){
		if (confirm('Are you sure you want to generate this Advances?\nPress OK'))
		{
			$.ajax({
				type : "POST",
				url : "<?php echo $this->Html->url(array('controller'=>'advances','action'=>'generate')); ?>",
