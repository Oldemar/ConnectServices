		...
		<tr>
			...
			<?
				foreach ($eventsByDay as $time => $dates) 
				{
					...
					{
						echo '<td class="droppable" data-date="'. date('Y-m-d',strtotime($date . "+$key days")) .'" data-start="'. gmdate('h:i:s', $time*1800) .'" data-end="'. $time .'" data-uid="" style="width: 13.25%; "><div style="width: 100%;">';
						$z=0;
						$w=120;
						foreach ($dateEvent as $keyEvent => $events) 
						{
							...
							echo "<div class=\"event ".$dgb."\" data-uid=\"".$events['User']['id']."\" data-eid=\"".$events['Event']['id']."\" style=\"float: left; color: #fff; background-color: ".$events['EventType']['color']."; width: 50%;\"><h4><small><u>".$events['User']['username'].'</u></h4>'.$events['Event']['title']."</small></div>";
						}
						echo '</div></td>';
					}
